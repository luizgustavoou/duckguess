...
export abstract class ThemeService {
  ...
  abstract findAll(): Promise<IThemeDto[]>;
...
export class ThemeServiceImpl implements ThemeService {
  constructor(private readonly themeRepository: ThemeRepository, private readonly themeRepositoryQuery: ThemeQueryRepository) { }

  async create(createThemeDto: CreateThemeDto): Promise<Theme> {
    return this.themeRepository.save({ value: createThemeDto.value });
  }

  async findAll(): Promise<IThemeDto[]> {
    return this.themeRepositoryQuery.findAllWithGuesses();
  }
